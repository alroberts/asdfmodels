<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow sm:rounded-lg p-6 border-2 border-black mb-6">
                ...
                <form method="POST" action="{{ route('admin.photographer-options.specialties.store') }}">
                    @csrf
            ...
            <div class="bg-white shadow sm:rounded-lg p-6 border-2 border-black">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($specialties as $specialty)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-900">{{ $specialty->key }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $specialty->label }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $specialty->display_order }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <button onclick="editSpecialty({{ $specialty->id }}, '{{ $specialty->key }}', '{{ $specialty->label }}', {{ $specialty->display_order }}, {{ $specialty->is_active ? 'true' : 'false' }})" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                                <i class="fas fa-edit"></i> Edit
                                            </button>
                                            <form method="POST" action="{{ route('admin.photographer-options.specialties.delete', $specialty->id) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this specialty? Users who have this specialty will no longer see it, but their data will remain intact.');">
                                                @csrf
                                                @method('DELETE')
                ...
                @else
                    <p class="text-gray-500">No specialties found. Add your first specialty above.</p>
                @endif
            </div>
    ...
    <script>
        function editSpecialty(id, key, label, displayOrder, isActive) {
            document.getElementById('editForm').action = '{{ route("admin.photographer-options.specialties.update", ":id") }}'.replace(':id', id);
